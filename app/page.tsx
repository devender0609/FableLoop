...
export default function Home(){
 const[idea,setIdea]=useState(""),[characters,setCharacters]=useState(""),[style,setStyle]=useState("Cinematic realism"),[format,setFormat]=useState("16:9"),[length,setLength]=useState("30"),[stage,setStage]=useState<Stage>("idea"),[plan,setPlan]=useState<Plan|null>(null),[movieUrl,setMovieUrl]=useState(""),[error,setError]=useState(""),[configured,setConfigured]=useState<boolean|null>(null),[renderMode,setRenderMode]=useState<RenderMode>("draft");
 ...
 useEffect(()=>{try{const saved=localStorage.getItem("fableloop-production-v22");if(saved){const value=JSON.parse(saved);if(value?.plan){setPlan(value.plan);setIdea(value.idea||"");setCharacters(value.characters||"");setStyle(value.style||"Cinematic realism");setFormat(value.format||"16:9");setLength(value.length||"30");setRenderMode(value.renderMode||"draft");setMovieUrl(value.movieUrl||"");setStage(value.movieUrl?"complete":"review")}}}catch{}},[]);
 useEffect(()=>{if(plan)try{localStorage.setItem("fableloop-production-v22",JSON.stringify({plan,idea,characters,style,format,length,renderMode,movieUrl}))}catch{}},[plan,idea,characters,style,format,length,renderMode,movieUrl]);
 ...
 async function render(){
  ...
  const shot=plan.shots.find(s=>s.status!=="ready");
  if(!shot){await mergeMovie(plan.shots.map(s=>s.videoUrl).filter(Boolean) as string[]);return}
  stop.current=false;setError("");setStage("rendering");
   try{
    updateShot(shot.id,{status:"queued"});
    const submit=await fetch("/api/movie/submit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:shot.prompt,dialogue:shot.dialogue,seconds:shot.seconds,format,mode:renderMode})});
    ...
    for(let attempt=0;attempt<120&&!done;attempt++){
     ...
     if(data.status==="COMPLETED"){if(!data.videoUrl)throw new Error("Provider completed without a video.");updateShot(shot.id,{status:"ready",videoUrl:data.videoUrl,error:undefined});done=true}
    ...
    setStage("review");
   }catch(e){updateShot(shot.id,{status:"failed",error:e instanceof Error?e.message:"Shot failed"});setError(`“${shot.title}” failed. Completed shots are safe. You can retry only this unfinished shot.`);setStage("error");return}
 }
  ...
  <div className="mx-auto grid max-w-[1380px] lg:grid-cols-[260px_1fr]">
   ...
   <section className="min-w-0 px-5 py-8 sm:px-8 lg:px-12 lg:py-12">
    <div className="mx-auto max-w-5xl">
     ...
     {(stage==="review"||stage==="rendering"||stage==="merging"||stage==="error")&&plan&&<Production plan={plan} setPlan={setPlan} stage={stage} progress={progress} ready={ready} total={total} onRender={render} renderMode={renderMode} setRenderMode={setRenderMode}/>}
...
function Production({plan,setPlan,stage,progress,ready,total,onRender,renderMode,setRenderMode}:{plan:Plan;setPlan:(p:Plan)=>void;stage:Stage;progress:number;ready:number;total:number;onRender:()=>void;renderMode:RenderMode;setRenderMode:(m:RenderMode)=>void}){
 const editable=stage==="review"||stage==="error",next=plan.shots.find(s=>s.status!=="ready"),remainingSeconds=plan.shots.filter(s=>s.status!=="ready").reduce((n,s)=>n+s.seconds,0),nextCost=next?next.seconds*MODE_INFO[renderMode].rate:0,totalCost=remainingSeconds*MODE_INFO[renderMode].rate;return <div className="grid gap-6 xl:grid-cols-[1fr_320px]"><div><div className="mb-5 rounded-2xl border border-white/10 bg-[#13151d] p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="text-2xl font-bold">{plan.title}</h2><p className="mt-2 text-sm leading-6 text-white/45">{plan.logline}</p></div><Badge className="bg-[#ff5d45]/15 text-[#ff8978] hover:bg-[#ff5d45]/15">{plan.style}</Badge></div></div><Accordion type="multiple" defaultValue={plan.shots.map(s=>String(s.id))} className="space-y-3">{plan.shots.map((shot,i)=><AccordionItem key={shot.id} value={String(shot.id)} className="rounded-2xl border border-white/10 bg-[#13151d] px-5"><AccordionTrigger className="hover:no-underline"><div className="flex min-w-0 flex-1 items-center gap-4 text-left"><ShotStatus status={shot.status}/><div className="min-w-0"><p className="truncate font-bold">Shot {i+1}: {shot.title}</p><p className="mt-1 text-xs text-white/35">{shot.seconds} seconds · {shot.dialogue?"Dialogue + native audio":"Action + native audio"}</p></div></div><ChevronDown className="ml-2 size-4 text-white/30"/></AccordionTrigger><AccordionContent><div className="grid gap-4 pb-3"><div><Label className="mb-2 block text-xs text-white/35">Cinematic prompt</Label><Textarea value={shot.prompt} disabled={!editable} onChange={e=>setPlan({...plan,shots:plan.shots.map(s=>s.id===shot.id?{...s,prompt:e.target.value}:s)})} className="min-h-28 border-white/10 bg-[#0d0f15] text-sm leading-6 text-white disabled:opacity-80"/></div>{shot.dialogue&&<div><Label className="mb-2 block text-xs text-white/35">Spoken dialogue</Label><Input value={shot.dialogue} disabled={!editable} onChange={e=>setPlan({...plan,shots:plan.shots.map(s=>s.id===shot.id?{...s,dialogue:e.target.value}:s)})} className="border-white/10 bg-[#0d0f15] text-white"/></div>}{shot.videoUrl&&<video src={shot.videoUrl} controls className="aspect-video w-full rounded-xl bg-black"/>}{shot.error&&<p className="text-xs text-red-300">{shot.error}</p>}</div></AccordionContent></AccordionItem>)}</Accordion></div>
 <aside className="space-y-4"><div className="rounded-2xl border border-white/10 bg-[#13151d] p-5"><p className="mb-3 text-xs font-bold uppercase tracking-wider text-white/35">Character continuity</p><p className="text-sm leading-6 text-white/60">{plan.characterBible}</p></div><div className="rounded-2xl border border-white/10 bg-[#13151d] p-5"><p className="mb-3 text-xs font-bold uppercase tracking-wider text-white/35">Cost control</p><Pick label="Render quality" value={renderMode} set={x=>setRenderMode(x as RenderMode)} values={["draft","standard","pro"]}/><p className="mt-2 text-xs text-white/45">{MODE_INFO[renderMode].label} · {MODE_INFO[renderMode].note}</p><div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-xl bg-white/5 p-3"><p className="text-[11px] text-white/35">Next shot</p><b>${nextCost.toFixed(2)}</b></div><div className="rounded-xl bg-white/5 p-3"><p className="text-[11px] text-white/35">All remaining</p><b>${totalCost.toFixed(2)}</b></div></div></div><div className="rounded-2xl border border-white/10 bg-[#13151d] p-5"><div className="mb-3 flex justify-between text-sm"><span>Production progress</span><b>{Math.round(progress)}%</b></div><Progress value={progress} className="h-2"/><p className="mt-3 text-xs text-white/35">{ready}/{total} cinematic shots complete · saved on this device</p></div><Button onClick={onRender} disabled={!editable} className="h-12 w-full bg-[#ff5d45] font-bold hover:bg-[#e84d37]">{next?<><Film/>Render next shot · ${nextCost.toFixed(2)}</>:<><Clapperboard/>Assemble movie</>}</Button><p className="text-center text-xs leading-5 text-white/30">Only one paid shot starts per click. Completed clips are skipped and never submitted again.</p></aside></div>
}
